...
public class MixinOnlineServerEntry {
    ...
    @Redirect(method = "render", at = @At(value = "INVOKE", ordinal = 0, target = "Lnet/minecraft/client/gui/GuiGraphics;blit(Lnet/minecraft/resources/ResourceLocation;IIFFIIII)V"))
    private void redirectPingIcon(GuiGraphics instance, ResourceLocation atlasLocation, int x, int y, float uOffset, float vOffset, int width, int height, int textureWidth, int textureHeight) {
        if (((ViaServerData) this.serverData).viaFabric$translating()) {
            atlasLocation = new ResourceLocation("viafabric:textures/gui/icons.png");
        }
